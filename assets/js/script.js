...
var taskFormHandler = function(event) {
    ...
    // assign true/false if formEl has a data-task-id attribute
...
var deleteTask = function(taskId) {
    ...
    // so pushes all objects we want to *keep* into new array, excluding the one we want to delete
    if (tasks[i].id !== parseInt(taskId)) {
...
var loadTasks = function() {
    ...
    // iterate through array to create tasks elements from it
    for(i=0; i < tasks.length; i++) {
        console.log(tasks[i]);
        // to keep the id for each task in sync, reassign id property of tasks[i]
        ...
        // create div element and append to list element
        var taskInfoEl = document.createElement("div");
        taskInfoEl.className = "task-info";
        taskInfoEl.innerHTML = "<h3 class='task-name'>" + tasks[i].name + "</h3><span class='task-type'>" + tasks[i].type + "</span>";
        listItemEl.appendChild(taskInfoEl);

        // create actions for task and append to list element
        var taskActionsEl = createTaskActions(tasks[i].id);
        listItemEl.appendChild(taskActionsEl);
        console.log(listItemEl);

        // check if value of tasks[i].status is equal to todo
